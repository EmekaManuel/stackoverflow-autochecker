...
def make_req(error):
    resp = requests.get("https://api.stackexchange.com/" +
                        "/2.3/search?order=desc&sort=activity&tagged=python&intitle={}&site=stackoverflow".format(error))
    return resp.json()
...
if __name__ == "__main__":
    op, err = execute_return('python3 test.py')
    error_message = err.decode('utf-8').strip().split('\r\n')[-1]

    print(error_message)
    if error_message:
        filter_err = error_message.split(':')
        json1 = make_req(filter_err[1])
        json2 = make_req(filter_err[2])
